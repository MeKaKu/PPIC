...
export default {
  ...
  created() {
    ...
    this.$api.msg.getAllReplyMsgCount(this.user.name).then((res)=>{
      this.total = res.data
    })
    this.getItemList()
  },
  methods: {
    getItemList(){
      ...
      this.$api.msg.getMsgList(this.user.uid,this.user.name,startIndex,this.pageSize,1).then((res)=>{
        //console.log('请求消息:',res)
        tmp = res.data
        tmp.forEach((item)=>{
          item.content = ' 回复了你：' + item.content
    ...
    updateAll(){
      this.$api.msg.updateMsg(this.user.uid,this.user.name,1).then(()=>{
